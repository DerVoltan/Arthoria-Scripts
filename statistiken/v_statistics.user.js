    ...
    $.each(boxcontent, function(ind, line){
      ...
      if(!onHorse && line.indexOf(" beendet das Gebet ") == -1 && ( (url.indexOf("p=market") == -1 && url.indexOf("p=auction") == -1 && url.indexOf("p=event") == -1 && url.indexOf("&usp=48") == -1) || manualInput) ){
        if(line.indexOf(" öffnet den Lederbeutel und erhält ")>-1){
          ...
          if(data["options"]["tracking"]["leatherpouches"] == true && leatherpouch != false){
            leatherpouchesDataInput(line);
            leatherpouchesDataInput(leatherpouch);
            leatherpouch = false;
          }
          return true;
        }
...
function includeStatisticsCSS(){
  var head, style, css;
  css = "#dataOverviewDiv{overflow:auto;}"+
  "table.statisticsTableVertical>tbody>tr>td.statisticsHeaderTd{text-align:left;}table.statisticsTableVertical>tbody>tr>td.statisticsSomeTd{text-align:right;}"+
  "table.statisticsTableHorizontal>tbody>tr>td.statisticsHeaderTd{text-align:center;}table.statisticsTableHorizontal>tbody>tr>td.statisticsSomeTd{text-align:center}";
  head = document.getElementsByTagName('head')[0];


  if($(head).find('link[rel="stylesheet"]').attr("href").trim() == "stylen.css"){ //old design (green)
    css += "#statisticsTable tr:nth-child(odd) td{background-color:#455645;} #statisticsTable tr:nth-child(even) td{background-color:#718e71;}";
    css += "div#layout-container > table > tbody > tr:nth-child(2) > td:nth-child(2){max-width:"+$("div.pad5LR").css("width")+"}";
  }else{//new design (scroll)(stylen2.css)
    css += "#statisticsTable tr:nth-child(odd) td{background-color:#dabc88} #statisticsTable tr:nth-child(even) td{background-color:#e7d3b1}";
  }
  if (!head) { return; }
  ...
  switch(section){
    ...
    case "research":
      sortRuleArray = [
        ...
        "Spruchrolle: kleiner Zeitsprung",
        "Spruchrolle: Metamorphose",
        "Spruchrolle: schwache Heilung",
        "Spruchrolle: leichte Heilung",
        "Spruchrolle: mittlere Heilung",
        "Spruchrolle: Erfahrung",
        "Spruchrolle: Vergessen",
        "Spruchrolle: globale Erfahrung",
        "Bündnisrolle: Erfahrungsraub",
        "Spruchrolle: Pfeilregen",
        "Spruchrolle: Beschwörung (schwach)",
        "Buch: Fleiß",
...
function insertToolbox(){
  ...
  //$("div.pad5LR").prepend(toolbox); //alternative, but slightly smaller
  $(toolbox).show("slow");
}
...
function optionsMenu(){
  ...
  var cOptionsMenuObj = {"Character":"Charakter", "Tracking":"Überwachung", "Prices":"Preise", "Administration":"Datenbank"}
  var isfirst = true;
  $.each(cOptionsMenuObj, function(key, trans){
    var button = createButton("options"+key+"Button", trans);
    $(button).click(function(){
      if($("#"+key.toLowerCase()+"Content").children().length != 0){
        ...
        $("#optionsContentDiv").hide("slow", function(){
          ...
          $("#optionsContentDiv").append(getOptionsContent(key.toLowerCase()));
          $("#optionsContentDiv").show("slow");
        });
      }
    });
    if(!isfirst){
      $(optionsMenuDiv).append(menuspacer);
    }else{
      isfirst = false;
    }
    $(optionsMenuDiv).append(button);
  });
...
function getOptionsContent(section){
  switch(section){
    case "character":
      ...
      var stableStatisticsStandardDatabase = standardDatabase();

      $.each(stableStatisticsStandardDatabase["options"]["character"], function(key, value){
        if($.inArray(key, ["isEndNode", "lastupdate", "scriptversion"]) == -1){
          ...
          if(value === false || value === true){
            var type = "checkbox";
            var checked;
            data["options"]["character"][key] === undefined ? checked = false : checked = isChecked(data["options"]["character"][key]);
          ...
          var newInput = createInput("options"+firstLetterUpperCase(key)+"Input", type, data["options"]["character"][key], checked);
    ...
    case "tracking":
      ...
      $.each(stableStatisticsStandardDatabase["options"]["tracking"], function(key, value){
        trackingCounter++;
      });
      ...
      $.each(stableStatisticsStandardDatabase["options"]["tracking"], function(key, value){
        if(key != "isEndNode"){
          ...
          if(value === false || value === true){
            var type = "checkbox";
            var checked;
            data["options"]["tracking"][key] === undefined ? checked = false : checked = isChecked(data["options"]["tracking"][key]);
            var newInput = createInput("tracking"+firstLetterUpperCase(key)+"Input", type, data["options"]["tracking"][key], checked);
            $(newInput).css("width", "10em");
            $(firsttd).append(getTranslation(key, "translation")+": ");
            $(secondtd).append(newInput);
          }
      ...
      $(exportDataButton).click(function(){
        if(data["options"]["character"]["lastupdate"]!=0){
          ...
          if(data["flags"]["garden"]["fertilizerTarget"] != "" || parseInt(data["flags"]["garden"]["harvestbonusFlag"]) < 0){
            ...
            if(isChecked($("#resetDatabaseCheckbox").attr("checked"))){
...
function statisticsContent(){
  ...
  var statisticsVerticalCheckbox = createInput("statisticsVerticalCheckbox", "checkbox", false , false);
  $(statisticsVerticalCheckbox).change(function(){
    $(select0).trigger("change");
  });
  var shortNameUsageCheckbox = createInput("shortNameUsageCheckbox", "checkbox", false, false);
  $(shortNameUsageCheckbox).change(function(){
    $(select0).trigger("change");
  });
  $(statisticsContent).append(statisticsMenuDiv)
  .append("Vertikale Ansicht: ").append(statisticsVerticalCheckbox)
  .append("<br>")
  .append("Kurze Namen:").append(shortNameUsageCheckbox)
  .append("<hr>");
...
function getStatisticsContent(somedata){
  ...
  var fullySortedArray = sortRuleArray;
  $.each(somedata, function(key, value){
    if($.inArray(key, sortRuleArray) == -1 && key != "isEndNode"){
      fullySortedArray.push(key);
    }
  });
  var statisticsTable = document.createElement("table");
  $(statisticsTable).attr("id", "statisticsTable");
  if(isChecked($("#statisticsVerticalCheckbox").attr("checked"))){
    $(statisticsTable).attr("class", "statisticsTableVertical");
  }else{
    $(statisticsTable).attr("class", "statisticsTableHorizontal");
  }
  ...
  switch($("#select0").val()){
    case "herbs":
      ...
      var goldperhour = 0;
      $.each(fullySortedArray, function(index, entry){
        if(somedata[entry]){
          var headertd = document.createElement("td");
          $(headertd).append(nbsp+"<b>"+getShortName(getTranslation(entry, "translation"))+"</b>"+nbsp);
          $(headertd).attr("class", "statisticsHeaderTd");
          $(headerrow).append(headertd);
          var sometd =  document.createElement("td");
          $(sometd).attr("class", "statisticsSomeTd");
          $(sometd).append(somedata[entry]);
          $(somerow).append(sometd);

          var stattd =  document.createElement("td");
          $(stattd).attr("class", "statisticsSomeTd");
          if(entry== "herbsHourCount"){
            $(stattd).append("<b>1/h</b>");
          }else if(entry== "herbsNothing"){
            $(stattd).append("("+round((parseInt(somedata[entry]) / parseInt(somedata["herbsHourCount"]))*100, 2).toFixed(2) +"%)");
          }else{
            goldperhour += parseInt(somedata[entry]) * parseInt(data["options"]["prices"][entry]);
            $(stattd).append(round((parseInt(somedata[entry]) / parseInt(somedata["herbsHourCount"])), 2).toFixed(2));
          }
    ...
    case "wilderness":
      $.each(fullySortedArray, function(index, entry){
        if(somedata[entry]){
          var headertd = document.createElement("td");
          $(headertd).append(nbsp+"<b>"+getTranslation(entry, "translation")+"</b>"+nbsp);
          $(headertd).attr("class", "statisticsHeaderTd");
          $(headerrow).append(headertd);
          var sometd =  document.createElement("td");
          $(sometd).attr("class", "statisticsSomeTd");
      ...
      $.each(fullySortedArray, function(index, entry){
        if(somedata[entry]){
          ...
          if(entry == "birdCount"){
            ...
            $(stattd).append(round((parseInt(somedata[entry])/parseInt(somedata["birdCount"]))*100, 2).toFixed(2) +"%");
          }
          $(statrow).append(stattd);
        }
      });
    ...
    case "altar":
      ...
      $.each(fullySortedArray, function(index, entry){
        if(somedata[entry]){
          ...
          $(chancestd).attr("class", "statisticsSomeTd");
          var avgtd =  document.createElement("td");
          $(avgtd).attr("class", "statisticsSomeTd");

          if(entry == "prayCount"){
            $(chancestd).append("<b>%:</b>");
            $(avgtd).append("<b>&empty;:</b>");

          }else if(entry == "prayEPAmount"){
            $(chancestd).append("");
            $(avgtd).append(round((parseInt(somedata[entry])/parseInt(somedata["prayEPCount"])), 2).toFixed(2));

          }else if(entry == "prayGoldAmount"){
            $(chancestd).append("");
            $(avgtd).append(round((parseInt(somedata[entry])/parseInt(somedata["prayGoldCount"])), 2).toFixed(2));

          }else if($.inArray(entry, ["prayEPHighest", "prayEPLowest", "prayGoldHighest", "prayGoldLowest"]) > -1){
            $(chancestd).append("");
            $(avgtd).append("");

          }else if($.inArray(entry, ["prayNothing", "prayEPCount", "prayGoldCount", "prayHealCount", "prayHarvestCount", "prayActionpointsCount", "prayProtbuffCount"]) > -1){
            $(chancestd).append(round((parseInt(somedata[entry])/parseInt(somedata["prayCount"]))*100, 2).toFixed(2)+"%");
            $(avgtd).append("");
          }
    ...
    case "horse":
      ...
      $.each(fullySortedArray, function(index, entry){
        if(somedata[entry]){
          ...
          }else if(entry == "horseEPAmount"){
            $(stattd).append(round((parseInt(somedata[entry])/parseInt(somedata["horseCount"])), 2).toFixed(2));
          }else if(entry == "horseEncounters"){
            $(stattd).append(round((parseInt(somedata[entry])/parseInt(somedata["horseCount"]))*100, 2).toFixed(2)+"%");
          }else{
            $(stattd).append("");
          }
    ...
    case "garden":
      ...
      $.each(fullySortedArray, function(index, entry){
        if(somedata[entry]){
          ...
          var longHerbsArray = ["Guljakbeere", "Jorugawurzel", "Soragepilz", "Kuragknolle", "Kurelblüte", "Tairanblatt", "Tolwarknopse"];
          var actualHerb = "";
          var transHelper = entry;
          for(var h = 0; h < herbsArray.length; h++){
            if(entry.indexOf(herbsArray[h])>-1){
              if(isChecked($("#shortNameUsageCheckbox").attr("checked"))){
                actualHerb = herbsArray[h];
              }else{
                actualHerb = longHerbsArray[h];
              }
              transHelper = entry.replace(herbsArray[h], "Herb");
            }
          }
          $(headertd).append("<b>"+getTranslation(transHelper, "translation")+actualHerb+"</b>");
          $(sometd).append(somedata[entry]);

          if(entry.indexOf("harvest")>-1 && ( entry.indexOf("Count")>-1 ||  entry.indexOf("Amount")>-1)){
            var chancestd =  document.createElement("td");
            var avgtd = document.createElement("td");
            $(chancestd).attr("class", "statisticsSomeTd");
            $(avgtd).attr("class", "statisticsSomeTd");

            if(entry == "harvestCount"){
              $(chancestd).append("<b>%:</b>");
              $(avgtd).append("<b>&empty;:</b>");

            }else if(entry.indexOf("Amount")>-1){
              $(chancestd).append("");
              var countername = entry.substring(0, entry.indexOf("Amount"));
              $(avgtd).append(round((parseInt(somedata[entry])/parseInt(somedata[countername+"Count"])), 2).toFixed(2));

            }else if(entry.indexOf("Count")>-1){
              $(chancestd).append(round((parseInt(somedata[entry])/parseInt(somedata["harvestCount"]))*100, 2).toFixed(2)+"%");
              $(avgtd).append("");
            }
            ...
            if((entry.indexOf("harvest")>-1 && (entry.indexOf("Highest")>-1 || entry.indexOf("Lowest")>-1) )){
              $(secondheaderrow).append(headertd);
              $(secondsomerow).append(sometd);
            }else if(entry.indexOf("fertilizerSuccess")>-1 || entry.indexOf("fertilizerFailure")>-1){
    ...
    case "leatherpouches":
      ...
      $.each(fullySortedArray, function(index, entry){
        if(somedata[entry]){
          ...
          var stattd =  document.createElement("td");
          $(stattd).attr("class", "statisticsSomeTd");
          if(entry== "leatherpouchesCount"){
            $(stattd).append("<b>&empty; &amp; %</b>");
          }else if(entry== "leatherpouchesDamaged"){
            $(stattd).append(round((parseInt(somedata[entry]) / parseInt(somedata["leatherpouchesCount"])*100), 2).toFixed(2)+"%");
          }else if(entry== "leatherpouchesGoldAmount"){
            $(stattd).append(round((parseInt(somedata[entry]) / parseInt(somedata["leatherpouchesCount"])), 2).toFixed(2));
          }else{
            $(stattd).append("");
          }
          $(statrow).append(stattd);
    ...
    case "research":
      ...
      $.each(fullySortedArray, function(index, entry){
        if(somedata[entry]){
          ...
          var stattd =  document.createElement("td");
          $(stattd).attr("class", "statisticsSomeTd");
          if(entry== "researchCount"){
            $(stattd).append("<b>%</b>");
          }else if(entry== "researchSparks"){
            $(stattd).append(round((parseInt(somedata[entry]) / parseInt(somedata["researchCount"])*100), 2).toFixed(2)+"%");
          }else{
            $(stattd).append(round((parseInt(somedata[entry]) / parseInt(somedata["researchCount"])*100), 2).toFixed(2)+"%");
          }
          $(statrow).append(stattd);
    ...
    case "phasecrystals":
      ...
      $(statisticsTable).append(statrow);
      $.each(fullySortedArray, function(index, entry){
        var energyshards = entry.substring(entry.indexOf("ES")+2, entry.length);
        var headertd = document.createElement("td");
        $(headertd).attr("class", "statisticsHeaderTd");
        $(headerrow).append(headertd);

        if(entry.indexOf("phasecrystalAttemptES")>-1){
        ...
        if(somedata[entry]>=0){
          $(sometd).append(somedata[entry]);
          if(entry.indexOf("phasecrystalFailES")>-1){
            $(stattd).append(round((parseInt(somedata[entry]) / parseInt(somedata["phasecrystalAttemptES"+energyshards]))*100, 2).toFixed(2)+"%");
          }else{
            $(stattd).append("");
          }
        }else{
    ...
    case "staffEnhancement":
      ...
      $(statisticsTable).append(statrow);
      $(headerrow).append("<td></td>");
      $(somerow).append('<td class:"statisticsHeaderTd"><b></b></td>');
      ...
      $.each(fullySortedArray, function(index, entry){
        var headertd = document.createElement("td");
        $(headertd).attr("class", "statisticsHeaderTd");
        var targetedlvl = parseInt(entry.substring(entry.indexOf("To")+2, entry.length));
        if(entry.indexOf("AttemptCount")>-1){
          $(headertd).append(nbsp+"<b>"+getTranslation("staffEnhancementAttemptCountTo", "translation")+targetedlvl+"</b>"+nbsp);
        }else if(entry.indexOf("SuccessCount")>-1){
        ...
        if(entry.indexOf("AttemptCount")>-1){
          ...
          if(somedata[entry]>=0){
            $(stattd).append(round((parseInt(somedata[entry])/parseInt(somedata["staffEnhancementAttemptCountTo"+targetedlvl]))*100, 2).toFixed(2)+"%");
          }else{
            $(stattd).append("");
          }
    ...
    case "glyphboards":
      ...
      $.each(fullySortedArray, function(index, entry){
        ...
        var stattd = document.createElement("td");
        $(stattd).attr("class", "statisticsSomeTd");
        var effective = entry.substring(0, entry.indexOf("#"));
        var attribute = entry.substring(entry.indexOf("#")+1, entry.length);

        if(somedata[entry]){
          $(sometd).append(somedata[entry]);
          $(stattd).append(round((parseInt(somedata[entry])/parseInt(somedata["glyphboardCount"]))*100, 2).toFixed(2)+"%");
        }else{
          $(sometd).append("");
          $(stattd).append("");
        }
    ...
    case "stoneOfPatience":
      ...
      $.each(fullySortedArray, function(index, entry){
        ...
        if(somedata[entry]>=0){
          $(headertd).append(nbsp+"<b>"+getTranslation(entry, "translation")+"</b>"+nbsp);
          $(sometd).append(somedata[entry]);
          if(entry == "sopAttemptCount"){
            $(chancestd).append(nbsp+"<b>%:</b>"+nbsp);
            $(avgtd).append(nbsp+"<b>&empty;:</b>"+nbsp);
          }else if(entry.indexOf("Count")>-1 && entry != "sopAttemptCount"){
            $(chancestd).append(round(parseInt(somedata[entry])/parseInt(somedata["sopAttemptCount"])*100, 2).toFixed(2) +"%");
            $(avgtd).append("");
          }else if(entry.indexOf("Amount")>-1){
            if(entry.indexOf("Glory")>-1){
              var soptype = "Glory";
            }else if(entry.indexOf("EP")>-1){
              var soptype = "EP";
            }
            $(chancestd).append("");
            $(avgtd).append(round(parseInt(somedata[entry])/parseInt(somedata["sop"+soptype+"Count"]), 2).toFixed(2));
          }else{
            $(chancestd).append("");
            $(avgtd).append("");
          }
    ...
    case "plumbumclump":
      ...
      var goldperclump = round((parseInt(somedata["plumbumclumpGoldAmount"]) / parseInt(somedata["plumbumclumpCount"])), 2).toFixed(2);
...
function createKeyOverviewDiv(){
  ...
  $(toggleKeyOverviewButton).html("&#8675;Legende"); //"fix"
  $(toggleKeyOverviewButton).click(function(){
    if($(keyOverviewSpan).css("display") == "none"){
      $(toggleKeyOverviewButton).html("&#8673;Legende") //arrow up - is open after this
    }else{
      $(toggleKeyOverviewButton).html("&#8675;Legende") //arrow down - is closed after this
    }
...
function modifyStatisticSelect(somedata, depth){
  if(somedata["isEndNode"]==1){
    ...
    $("#statisticsContentDiv").append("<br>").append(createKeyOverviewDiv());
    if(isChecked($("#statisticsVerticalCheckbox").attr("checked"))){
      ...
      $("#statisticsTable td").each(function(){
        if($(this).html().indexOf("<b>") >- 1){
          $(this).attr("class", "statisticsHeaderTd");
        }else{
          $(this).attr("class", "statisticsSomeTd");
        }
      });
    }
...
function getShortName(selector){
  ...
  if(isChecked($("#shortNameUsageCheckbox").attr("checked")) || $("#select0").length == 0){ //test..
...
function getGlyphboardType(id){
  ...
  if(allGlyphboards[parseInt(id)]){
    var glyphboardType = allGlyphboards[parseInt(id)];
  }else{
...
function saveOptions(){
  if ( (isChecked($("#optionsHuntInput").attr("checked")) && isChecked($("#optionsFaithInput").attr("checked"))) ||
       (isChecked($("#optionsHuntInput").attr("checked")) && isChecked($("#optionsGardeningInput").attr("checked"))) ||
       (isChecked($("#optionsFaithInput").attr("checked")) && isChecked($("#optionsGardeningInput").attr("checked"))) ){
    alert("Von Gartenbau, Glaube und Jagd kann nur eines aktiv sein. Die Einstellungen wurden nicht übernommen.");
  }else if(isChecked($("#optionsLibrarianInput").attr("checked")) && !isChecked($("#optionsSpotScrollsInput").attr("checked"))){
    ...
    $.each(stableStatisticsStandardDatabase["options"]["character"], function(key, value){
      if($.inArray(key, ["isEndNode", "lastupdate", "scriptversion"]) == -1){
        if($("#options"+firstLetterUpperCase(key)+"Input").attr("type") == "checkbox"){
          data["options"]["character"][key] = isChecked($("#options"+firstLetterUpperCase(key)+"Input").attr("checked"));
  ...
  $.each(stableStatisticsStandardDatabase["options"]["tracking"], function(key, value){
    if(key != "isEndNode"){
      if($("#tracking"+firstLetterUpperCase(key)+"Input").attr("type") == "checkbox"){
        data["options"]["tracking"][key] = isChecked($("#tracking"+firstLetterUpperCase(key)+"Input").attr("checked"));
      }else{
...
function birdDataInput(line){
  ...
  if(line.indexOf("Begleiter findet nichts")>-1){
    ...
    if(line.indexOf(" eine ")>-1 || line.indexOf(" ein ") >-1){
      var count = 1;
  ...
  if(data["flags"]["garden"]["harvestbonusFlag"] >= 0){
    if(line.indexOf(" ist mit der Gartenarbeit fertig")>-1 || line.indexOf(" düngt den Garten und versucht ")>-1){
      ...
      if(line.indexOf(".")>-1){
        ...
        var herb = line.substring(line.indexOf("x ")+2, line.length());
      }
...
function statisticsInternalVersioncontrol(){
  if("v"+statisticsVersion != data["options"]["character"]["scriptversion"]){
    ...
    if(installedversion < 1.2){
      var wrongentryarray = ["Tairanblatt", "Kuragknolle", "Kurelblüte", "Heiltrank", "alte Silbermünze", "Lederbeutel", "Erzklumpen", "Manakristall"];
      var wrongentryarraybig = [];
      for (var i=0; i < wrongentryarray.length; i++){
        wrongentryarraybig.push("det 1x "+wrongentryarray[i]);
      }
      deleteDatabaseEntries(data["options"]["prices"], wrongentryarraybig);
      if(data["bird"]){
        fix1p2(data["bird"], wrongentryarray, wrongentryarraybig);
      }
      installedversion = 1.2;
    }
    if(installedversion < 1.4){
      if(data["flags"]["leatherpouches"]){
        delete data["flags"]["leatherpouches"];
      }
      if(data["garden"]){
        fix1p3(data["garden"]);
      }
...
function deleteDatabaseEntries(section, array){
  if(section["isEndNode"] == 1){
    ...
    $.each(section, function(key, value){
      deleteDatabaseEntries(value, array);
    });
  }
}

function fix1p2(section, array, arraybig){
  if(section["isEndNode"] == 1){
    for(var i=0; i < array.length; i++){
      if(section[arraybig[i]]){
        if(!section[array[i]]){
          section[array[i]] = 0;
        }
        section[array[i]] = parseInt(section[array[i]]) + parseInt(section[arraybig[i]]);
        delete section[arraybig[i]];
      }
    }
  }else{
    $.each(section, function(key, value){
      fix1p2(value, array, arraybig);
    });
  }
}

function fix1p3(section){
  if(section["isEndNode"] == 1){
    $.each(section, function(key, value){
      if(key.indexOf(" erntet ")>-1){
        delete section[key];
      }
    });
    var nhc = 0;
    ...
    var nffc = 0;
    $.each(section, function(key, value){
      if(key.indexOf("Count")>-1 && key.indexOf("harvest")>-1 && key != "harvestCount"){
        nhc += parseInt(value);
      }else if(key.indexOf("fertilizerSuccess")>-1 && key != "fertilizerSuccessCount"){
        nfsc += parseInt(value);
      }else if(key.indexOf("fertilizerFailure")>-1 && key != "fertilizerFailureCount"){
        nffc += parseInt(value);
      }
    });
    if(section["harvestCount"]){
      section["harvestCount"] = nhc;
    }
    if(section["fertilizerSuccessCount"]){
      section["fertilizerSuccessCount"] = nfsc;
    }
    if(section["fertilizerFailureCount"]){
      section["fertilizerFailureCount"] = nffc;
    }
  }else{
    $.each(section, function(key, value){
      fix1p3(value);
